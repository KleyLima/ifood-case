...
def download_file_stream_and_upload(
            ...
            if chunk:
                try:
                    ...
                    if total_size:
                        ...
                        print(
                            f"\rProgress: {percent:.1f}% ({downloaded / (1024*1024):.2f}/{total_size / (1024*1024):.2f} MB)",
                            end="",
                        )
                    else:
                        print(
                            f"\rUploaded part {part_number - 1}, total size: {downloaded / (1024*1024):.2f} MB",
                            end="",
                        )
                    ...
                    s3.abort_multipart_upload(
                        Bucket=bucket_name, Key=s3_file_key, UploadId=upload_id
                    )
                    raise
...
def all_download_combinations(start_date, end_date):
    ...
    combinations = [
        f"{taxi}_tripdata_{data}.parquet"
        for taxi, data in product(TAXI_TYPES, date_combinations)
    ]
